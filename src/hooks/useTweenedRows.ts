...
/** Interpolate prev->next rows for dur ms. Keeps exactly N rows by resorting each frame. */
export function useTweenedRows(prev: Slice, next: Slice, n = 10, dur = 900) {
  const [frameRows, setFrameRows] = useState<Slice>(next);

  // Build union of names so entering/exiting countries slide in/out smoothly.
  const names = useMemo(() => {
  ...
  useEffect(() => {
    ...
    const tick = (t: number) => {
      ...
      // Interpolate each country value
      const blended = names.map((name) => {
        const a = prevMap.get(name) ?? 0; // carry-over if absent
        const b = nextMap.get(name) ?? 0;
        return { name, pop: Math.round(a + (b - a) * k) };
      });

      // Resort by current value and keep exactly N
